  ...
  const handleKeyDown = (e) => {
    ...
    if (open) {
      if (e.key === "ArrowDown") {
        ...
        if (selected) {
          onChange({
            target: { name: "roleId", value: selected.id, roleName: selected.name, roleType: selected.type }
          });
          setOpen(false);
          if (onMoveNext) {
            setTimeout(() => onMoveNext());
          }
        }
        e.preventDefault();
      }
  ...
  return (
    <div className="relative group" ref={containerRef}>
      <button
        ...
        name="roleId"
        onClick={() => setOpen((x) => !x)}
        onFocus={() => { if (!open) setOpen(false); }}
        onBlur={() => { setTimeout(() => setOpen(false), 120); }}
        onKeyDown={handleKeyDown}
        className={`w-full text-left border-0 border-b-2 border-blue-300 focus:border-blue-600 bg-transparent px-0 py-3 text-lg font-medium text-gray-800 transition-all duration-300 cursor-pointer min-h-[27px] outline-none ${
          !value ? "text-gray-400" : ""
        }`}
        ...
        required
      >
        {value && options.some((r) => "" + r.id === "" + value)
          ? options.find((r) => "" + r.id === "" + value).name
          : "Select Role"}
      </button>
      <label
        id="roleIdLabel"
          ...
          {options.map((r, i) => (
            <li key={r.id}>
              <button
                ...
                tabIndex={-1}
                type="button"
                className={`w-full text-left px-4 py-2 text-base hover:bg-blue-50 ${
                  i === highlight ? "bg-blue-100 text-blue-800 font-bold" : ""
                }`}
                onMouseEnter={() => setHighlight(i)}
                onClick={() => {
                  onChange({
                    target: { name: "roleId", value: r.id, roleName: r.name, roleType: r.type }
                  });
                  setOpen(false);
...
function SignupFormPart1({ form, focus, loading, error, onChange, onNext, onFocus, onBlur, onInputKeyDown, roles, roleRef, usernameRef, emailRef, passwordRef }) {
  return (
...
function SignupFormPart2({ form, focus, loading, error, onChange, onSignup, onFocus, onBlur, onInputKeyDown, fullNameRef, schoolNameRef, idNumberRef, classOrPositionRef, onBack, success }) {
  return (
    <div className="space-y-7">
        <div className="relative group">
            <input
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button
            type="submit"
            className={`flex-1 py-3 rounded-xl font-extrabold text-lg transition-all duration-300 bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-500 text-white shadow-xl hover:from-blue-700 hover:to-cyan-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 ${
