...
export async function POST(req) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, message: "Email, OTP, and new password are required (and must be non-empty strings)" },
        { status: 400 }
      );
    }
    ...
    if (!userEmail && user.warehouse?.email) userEmail = user.warehouse.email;
    if (!userEmail && user.store?.email) userEmail = user.store.email;
    if (userEmail) {
      await sendPasswordResetEmail(user, userEmail);
    }

    return NextResponse.json(
      { success: true, message: "Password reset successfully in UserLogin" },
      { status: 200 }
    );
  } catch (error) {
