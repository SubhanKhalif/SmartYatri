...
function NotificationFullScreen({
  ...
  useEffect(() => {
    if (showNotifications && notifMainRef.current) {
      notifMainRef.current.focus();
    }
  }, [showNotifications]);
...
// Helper hook to provide page padding for mobile to prevent content from hiding behind header/footer
export function useMobileSafeArea() {
  useEffect(() => {
    // set attribute/class to <body> as a CSS hook for mobile safe area padding
    document.body.classList.add('sy-mobile-safearea');
    return () => {
      document.body.classList.remove('sy-mobile-safearea');
    };
  }, []);
}

// Styles for safe area: add in global CSS, but for inline demo, we create a <style> tag
// This is a best-effort: the actual safe area (env variables) may need additional setup for full cross-device coverage
if (typeof window !== 'undefined' && !document.getElementById('sy-mobile-safearea-style')) {
  const style = document.createElement('style');
  style.id = 'sy-mobile-safearea-style';
  style.innerHTML = `
    @media (max-width: 767px) {
      body.sy-mobile-safearea {
        /* Account for header (top) and nav (bottom) heights (both h-14; 56px each), plus iOS safe area insets */
...
export default function Sidebar({ role = 'student' }) {
  ...
  // --- Add safe area hook so any page using Sidebar gets safe padding on mobile ---
  useMobileSafeArea();
  ...
  const menuItems = {
    student: [
      // ... [KEEP menuItems THE SAME AS ORIGINAL]
      { path: '/student/dashboard', label: 'Dashboard', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg> )},
      { path: '/student/book-ticket', label: 'Buy Ticket', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" /></svg> )},
      { path: '/student/my-tickets', label: 'My Tickets', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> )},
      { path: '/student/my-pass', label: 'My Pass', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" /></svg> )},
      { path: '/student/travel-history', label: 'Travel History', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg> )},
      { path: '/student/profile', label: 'Profile', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg> )},
    ],
    conductor: [
      { path: '/conductor/scanner', label: 'QR Scanner', icon: ( <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg> )},
    ],
    // ... [KEEP menuItems for manager, admin, faculty as per original code]
    manager: [ /* ... */ ],
    admin: [ /* ... */ ],
    faculty: [ /* ... */ ],
  };
  ...
  // No longer close notification on outside click for fullscreen, handled in overlay click
  ...
  return (
    <>
      {/* Professional Fullscreen Notification Overlay (used for all screen sizes) */}
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 bg-black border-b border-gray-800">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-3">
            {/* Notification icon always opens fullscreen */}
            <button
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black border-t border-gray-800 safe-area-inset-bottom">
      ...
      <div className="hidden md:block">
        <aside
          className="sticky top-0 h-screen w-64 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white flex flex-col shadow-2xl border-r border-gray-700"
        >
          <div className="p-6 border-b border-gray-700/50 bg-gradient-to-r from-gray-800 to-gray-900">
            <div className="flex items-center justify-between mb-2">
              ...
              {/* Notification icon always opens fullscreen panel */}
              <button
                onClick={() => setShowNotifications(true)}
